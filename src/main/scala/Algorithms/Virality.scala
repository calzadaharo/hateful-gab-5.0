...
class Virality extends GraphAlgorithm {
  override def apply(graph: GraphPerspective): GraphPerspective = {
    graph
      .step(vertex => {
        val index = vertex.getPropertyOrElse[Long]("index",null)
      ...
      .iterate({vertex => {
        ...
        messages.foreach( message => {
          ...
          val my_index = vertex.getPropertyOrElse[Long]("index",null)
          val my_sent = vertex.getPropertyOrElse[Long]("sent",null)

          // Check if I am the main vertex
          if (my_index == received_index) {
            // Check if it comes from an specific vertex
            if (received_index == 0) {
              val my_new_storage = vertex.getPropertyOrElse[Long]("storage",null) + 1
              val my_new_sum = vertex.getPropertyOrElse[Long]("storage",null) +
                vertex.getPropertyOrElse("distance",null)
